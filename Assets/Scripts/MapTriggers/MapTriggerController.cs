...
public class MapTriggerController : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (this.name == "MapTrigger2" && IsObject() && other.gameObject.CompareTag("Player"))
        {
            ...
            dialogueManager.GetComponent<DialogueManager>().PlayerSay("[-EEVL_I-]: Its utility is beyond mine and your understanding, and we are gonna use it to get out of here.", 12, false);
            dialogueManager.GetComponent<DialogueManager>().PlayerSay("[-EEVL_I-]: Here I'll open the next door for you.", 22, false);
            dialogueManager.GetComponent<DialogueManager>().PlayerSay("", 28, false);
            this.GetComponent<BoxCollider>().enabled = false;
        }
        ...
        {
            StartCoroutine(DoorTrigger(object1, 28));
            ...
            dialogueManager.GetComponent<DialogueManager>().PlayerSay("[-EEVL_I-]: Don't worry I'll still be here, I wanna see that device in full action!", 22, false);
            dialogueManager.GetComponent<DialogueManager>().PlayerSay("[-EEVL_I-]: What a miraculous device we've stumbled upon...", 32, false);
            this.GetComponent<BoxCollider>().enabled = false;
        }
    }
}